...
struct WelcomeView: View {
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                ZStack {
                    ...
                    Circle()
                        .stroke(KickIQTheme.accent.opacity(0.15), lineWidth: 1.5)
                    ...
                    Circle()
                        .fill(
                            RadialGradient(
                                colors: [KickIQTheme.accent.opacity(0.3), KickIQTheme.accent.opacity(0.05), .clear],
                                center: .center,
                                startRadius: 20,
                                endRadius: 100
                            )
                    ...
                    Image(systemName: "soccerball")
                        ...
                        .opacity(logoOpacity)
                        .shadow(color: KickIQTheme.accent.opacity(0.5), radius: 20, x: 0, y: 0)
                }
                ...
                VStack(spacing: 6) {
                    HStack(spacing: 6) {
                        Circle().fill(KickIQTheme.accent).frame(width: 4, height: 4)
                        Circle().fill(KickIQTheme.accent.opacity(0.5)).frame(width: 4, height: 4)
                        Circle().fill(KickIQTheme.accent.opacity(0.3)).frame(width: 4, height: 4)
                    }
                    .opacity(subtitleOpacity)
                }
                .padding(.bottom, 60)
    ...
    private var welcomeBackground: some View {
        ZStack {
            ...
            RadialGradient(
                colors: [KickIQTheme.accent.opacity(0.06), .clear],
                center: .center,
                startRadius: 50,
                endRadius: 400
            )
